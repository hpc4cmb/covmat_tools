...
!    out from the matrix
!
! The resulting matrix must be subtracted from our (LNL^t)^{-1} to
! compute chi^2 values that do not have contribution from the 
! ill-conditioned eigenmode

PROGRAM get_reg_element
  ...
  REAL(dp)                   :: chisq
  REAL(dp)                   :: dof, monopole
  LOGICAL                    :: there, twomaps=.false.
  INTEGER(i4b)               :: ierr, isig1, isig2, nhit
  ...
  write (*,*) 'Without projection, the bad eigenmode has chi^2 = ', chisq
  ...
  DO icol = 0, npix*nstokes - 1
     READ(covmat_unit, rec=icol+1) covmat
     do irow = 0, npix*nstokes - 1
        covmat(irow) = covmat(irow) - map2(irow)*map2(icol)
     end do
